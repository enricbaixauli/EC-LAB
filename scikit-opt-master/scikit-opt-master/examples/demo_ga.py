...
for param_name, values in experiments.items():
    ...
    for val in values:
        ...
        for i in range(30):
            ga = GA_t(func=schaffer, n_dim=2, max_iter=100, lb=[-100, -100], ub=[100, 100], 
                      **current_params)
            best_x, best_y = ga.run()
            best_fitnesses.append(best_y)
        ...
        print(f"Config: {param_name}={val} -> Avg: {avg_fitness:.6f}, Std: {std_fitness:.6f}")
...
Y_history = pd.DataFrame(results)
